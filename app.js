...
document.addEventListener("DOMContentLoaded", () => {
  ...
  (() => {
    ...
    Animal.prototype.compareHeight = function () {
      ...
      return `A ${this.species} is ${Math.abs(
        heightDiff
      )} inches ${heightComp} than ${human.name}`;
    };
    ...
    Animal.prototype.compareWeight = function () {
      ...
      return `A ${this.species} is ${Math.abs(
        weightDiff
      )} pounds ${weightComp} than ${human.name}`;
    };
    ...
    Animal.prototype.compareDiet = function () {
      const dietComp =
        this.diet === human.diet
          ? `just like ${human.name}`
          : `while ${human.name} is a ${human.diet}`;
    ...
    Animal.prototype.getTileHTML = function () {
      const tileTitle =
        this.species === "human"
          ? `<h3>${this.name}</h3>`
          : `<h3>${this.species}</h3>`;
      const tileImg = `<img src="${this.getImageUrl()}" alt="Image rendering of a ${
        this.species
      }">`;
      const tileFact =
        this.species === "human" ? `` : `<p>${this.getRandomFact()}</p>`;
